...
function hasValidMacroQuad(){
  if(!MACRO_QUAD) return false;
  const g=MACRO_QUAD.growth || {};
  const i=MACRO_QUAD.inflation || {};
  const states=['Goldilocks','Reflation','Stagflation','Deflation'];
  return states.includes(MACRO_QUAD.currentState)
    && ['positive','negative'].includes(g.label)
    && ['positive','negative'].includes(i.label)
    && Number(g.score || 0) !== 0
    && Number(i.score || 0) !== 0;
}
function renderMacroQuadSnapshot(){
  ...
  if(!box) return;
  if(!hasValidMacroQuad()){
    box.innerHTML = `<div class="tiny-label">Growth / Inflation Regime</div><div class="text-sm text-slate-400 mt-1">Regime snapshot needs a fresh workflow run. The app will not publish positive or negative labels with zero scores.</div>`;
    return;
  }
  const g=MACRO_QUAD.growth || {};
  const i=MACRO_QUAD.inflation || {};
  const state=MACRO_QUAD.currentState;
  ...
  box.innerHTML = `<div class="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-4">
    ...
    <div class="macro-quad-axis-grid macro-quad-axis-grid-simple">
      <div class="macro-axis-card"><div class="tiny-label">Growth pressure</div><div class="metric-value text-lg font-bold ${axisClass(g.label)}">${axisWord(g.label, 'Positive', 'Negative')}</div><div class="text-xs text-slate-400 mt-1">Score ${fmtScore(Number(g.score))}</div></div>
      <div class="macro-axis-card"><div class="tiny-label">Inflation pressure</div><div class="metric-value text-lg font-bold ${axisClass(i.label)}">${axisWord(i.label, 'Positive', 'Negative')}</div><div class="text-xs text-slate-400 mt-1">Score ${fmtScore(Number(i.score))}</div></div>
    </div>
  </div>`;
}
...
function generateRegimeBrief(){
  ...
  const quad = hasValidMacroQuad() ? `- Current regime: ${MACRO_QUAD.currentState || ''} — ${MACRO_QUAD.subtitle || ''}\n- Growth pressure: ${fmtScore(MACRO_QUAD.growth?.score)} (${MACRO_QUAD.growth?.label})\n- Inflation pressure: ${fmtScore(MACRO_QUAD.inflation?.score)} (${MACRO_QUAD.inflation?.label})\n- Read: ${MACRO_QUAD.simpleRead || ''}` : '- Growth / Inflation Regime requires a fresh workflow run.';
  return `# Edgefield Research Macro Regime Brief v0.50.5\n\nGenerated: ${now}\n\nThis brief ranks public-source fundamental pressure evidence using uncapped raw net pressure scores. It is not a buy/sell signal and does not predict immediate price movement. Missing, stale, candidate, and display-only rows are not treated as neutral. v0.50.5 preserves the U.S.-centered, raw-score, no-price scanner while fixing the Growth / Inflation Regime map so the two visible axis scores are real signed tallies from normalized public-source macro observations.\n\n## Growth / Inflation Regime\n${quad}\n\n## Strongest positive raw pressure\n${topPos.map(fmt).join('\n') || '- No positive live-scored assets.'}\n\n## Strongest negative raw pressure\n${topNeg.map(fmt).join('\n') || '- No negative live-scored assets.'}\n\n## Conflicted / neutral candidates\n${conflicted.map(fmt).join('\n') || '- No conflicted or neutral candidates in current view.'}\n\n## Largest score changes\n${changes}\n\n## Source health\n${sourceLines || '- No source status loaded.'}\n\n## Upcoming tracked reports\n${events}\n\n## Caveats\n- Public-source macro/fundamental pressure only.\n- Scores are uncapped raw net pressure; larger absolute numbers mean more weighted evidence, not guaranteed price movement.\n- Mostly U.S.-based inputs, so relevance varies by asset.\n- Scores depend on current source freshness, row eligibility, and asset-specific mapping.\n- Open each asset row in the terminal to inspect counted, context, and excluded evidence.\n`;
}
...
$('exportJson').addEventListener('click',()=>download('macro_regime_scanner_public_source_data_contract_v0_50_5.json',JSON.stringify({notice:'Public-source data contract. v0.50.5 is the real quad tally baseline: U.S.-centered, raw-score, price-free macro pressure research data plus a no-price Growth / Inflation Regime map whose axis labels come directly from signed growth and inflation scores.',assets:ASSETS,source_status:SOURCE_STATUS,release_calendar:RELEASE_CALENDAR,release_results:RELEASE_RESULTS,source_quality:SOURCE_QUALITY,validation_summary:VALIDATION_SUMMARY,regime_bridge:REGIME_BRIDGE,macro_quad:MACRO_QUAD},null,2)));
const briefBtn=$('exportBrief');
if(briefBtn) briefBtn.addEventListener('click',()=>download('edgefield_macro_regime_brief_v0_50_3.md', generateRegimeBrief(), 'text/markdown'));
